...
x_value_sigma <- which.min(abs(ps - (mean_posterior + sqrt(variance_posterior))))
arrows(mean_posterior, dbeta(ps[x_value_sigma], alpha_posterior, beta_posterior), ps[x_value_sigma], dbeta(ps[x_value_sigma], alpha_posterior, beta_posterior), code = 3, col = "red", lwd = 1.2, length = 0.075)
text((mean_posterior + ps[x_value_sigma]) / 2, dbeta(ps[x_value_sigma], alpha_posterior, beta_posterior) - 0.8, expression(sigma), col = "red")
```
...
***(b) find the 95% credibility interval for μ***
